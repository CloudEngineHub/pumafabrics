...
LOAD_RESULTS = False
cases = ["PUMA", "PUMA_obst", "GF", "GM", "CM"]
results = {"min_distance": [], "collision": [], "goal_reached": [], "time_to_goal": [], "xee_list": [],
           "dist_to_NN": [], "dist_to_NN_std": [], "solver_times": [], "solver_time": [], "solver_time_std": []}
q_init_list = [
            np.array((-0.702, 0.355, -0.016, -1.212, 0.012, -0.502, -0.010)),
            np.array((0.07, 0.14, -0.37, -1.81, 0.46, -1.63, -0.91)),
            np.array((-0.302, 0.5, -0.016, -2, 0.012, -0.502, -0.010)),
            ...
            np.array((-0.60, 0.10, -0.016, -1.212, 0.012, -0.502, -0.010)),
            np.array((0.87, 0.14, -0.37, -1.81, 0.46, -1.63, -0.91)),
            np.array((-0.002, 0.355, -0.016, -1.212, 0.012, -0.002, -0.010))
        ]
positions_obstacles_list = [
    [[0.5, -0.25, 0.5], [0.24, 0.45, 0.2]],
    [[0.5, -0.2, 0.45], [0.24, 0.45, 0.2]],
    [[0.6, -0.1, 0.6], [0.24, 0.45, 0.2]],
    [[0.5, -0.2, 0.3], [0.24, 0.45, 0.2]],
    [[0.5, -0.0, 0.5], [0.3, -0.1, 0.5]],
    [[0.5, -0.3, 0.5], [0.5, 0.2, 0.25]],
    [[0.5, -0.4, 0.5], [0.5, 0.2, 0.4]],
    [[0.5, -0.15, 0.55], [0.24, 0.45, 0.2]],
    [[0.5, -0.4, 0.5], [0.5, 0.2, 0.4]],
    [[0.5, -0.2, 0.5], [0.24, 0.45, 0.2]],
]
...
def KukaComparison():
    ...
    results_stableMP_obst = run_i(class_SMP_obst, case=cases[0], q_init_list=q_init_list, results_stableMP=None)
...
def table_results(results):
    ...
    rows = []
    title_row = [' ','Success-Rate', 'Time-to-Success [s]', "Collision-rate","Min Clearance [m]", "Computation time [ms]", 'Path difference to SMP [m]']
    ...
    for case in cases:
        ...
        if results[case]["min_distance"] == [1000]*len(results[case]["min_distance"]):
            ...
            min_clearance_str = str(np.round(np.nanmean(results[case]["solver_times"]) * 1000, decimals=2)) + "+-" + str(
                np.round(np.nanstd(results[case]["solver_times"]) * 1000, decimals=2))

        rows.append([case,
                     str(np.sum(results[case]["goal_reached"]) / n_runs), #+ "+-" + str(np.round(np.nanstd(results[case]["goal_reached"]), decimals=4)),
                     str(np.round(np.nanmean(results[case]["time_to_goal"]), decimals=4)) + "+-" + str(np.round(np.nanstd(results[case]["time_to_goal"]), decimals=4)),
                     collision_episodes_rate_str,
                     min_clearance_str,
                     str(np.round(np.nanmean(results[case]["solver_times"])*1000, decimals=2)) + "+-" + str(np.round(np.nanstd(results[case]["solver_times"])*1000, decimals=2)),
                     str(np.round(np.nanmean(np.concatenate(results[case]["dist_to_NN"], axis=0)), decimals=2)) + "+-" + str(np.round(np.nanstd(np.concatenate(results[case]["dist_to_NN"], axis=0)), decimals=2)),
                     ])
    table = Texttable()
