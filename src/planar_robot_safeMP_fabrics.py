...
class example_planar_stableMP():
    ...
    def run_panda_example(self, n_steps=2000, env=None, goal=None, init_pos=np.array([-0.1, -0.7]), goal_pos=[0.8, 1.1], mode="acc", mode_NN="2nd", dt=0.01):
        # --- parameters --- #
        dof = 2
        dim_task = 2
        # mode = "vel"
        ...
        geometry_safeMP.set_limits(v_min=np.array([-50, -50]), v_max=np.array([50, 50]), acc_min=np.array([-50, -50]), acc_max=np.array([50, 50]))
        ...
        for w in range(n_steps):
            ...
            arguments_dict = dict(q=ob_robot["joint_state"]["position"],
                qdot=ob_robot["joint_state"]["velocity"],
            ...
            M, f, action_forced, qddot_speed = planner.compute_M_f_action_avoidance(**arguments_dict)
        ...
        plt.savefig(params.results_path+"images/planar_robot_safeMPfabrics_plot")
