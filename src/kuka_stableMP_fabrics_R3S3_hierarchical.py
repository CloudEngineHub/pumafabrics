...
class example_kuka_stableMP_R3S3():
    ...
    def run_kuka_example(self): #, n_steps=2000, env=None, goal=None, init_pos=np.array([0.0, 0.0, 0.]), goal_pos=[-0.24355761, -0.75252747, 0.5], mode="acc", mode_NN = "1st", dt=0.01, nr_obst=0):
        # --- parameters --- #
        # dof = 7
        # dim_pos = 3
        # dim_task = 7
        ...
        fk_full = self.forward_kinematics.casadi(q=planner._variables.position_variable(),
                                                 parent_link="iiwa_link_0",
                                                 child_link="iiwa_link_7")
        geometry_safeMP = construct_IL_geometry(planner=planner, dof=dof, dimension_task=self.params["dim_task"], forwardkinematics=fk_full,
        ...
        geometry_safeMP.set_limits(v_min=-np.array(self.params["vel_limits"]), v_max=np.array(self.params["vel_limits"]), acc_min=-50*np.ones((dof,)), acc_max=50*np.ones((dof,)))
        ...
        if params_name[0] == '2':
            ...
            x_t_init = np.array([np.append(np.append(x_function(q_init).full()[0:self.params["dim_pos"]], x_orientation), xdot_init)])
        else:
            x_t_init = np.array([np.append(x_function(q_init)[0:self.params["dim_pos"]], x_orientation)])
        ...
        # Translation of goal:
        normalizations = normalizaton_sim_NN(scaling_room=self.params["scaling_room"], dof_task=self.params["dim_task"])
        state_goal = np.array((self.goal._sub_goals[0]._config["desired_position"]))
        orientation_goal = np.array([1., 0., 0., 0.]) #np.array([0.0726, -0.6326, 0.3511, -0.6865]) #np.array([1., 0., 0., 0.])
        goal_normalized = np.append(normalizations.call_normalize_state(state=state_goal[0:self.params["dim_pos"]]), orientation_goal)
        ...
        list_fabr_avoidance = []
        list_safeMP = []
        q_list = np.zeros((dof, self.params["n_steps"]))
        x_list = np.zeros((self.params["dim_pos"], self.params["n_steps"]))
        ...
        for w in range(self.params["n_steps"]):
            ...
            # --- End-effector state ---#
            x_ee = x_function(q).full().transpose()[0][0:self.params["dim_task"]]
            ...
            if params_name[0] == "1":
                ...
                xdot_t = xdot_function_analytical(q, qdot).full().transpose()[0][0:self.params["dim_task"]]
                # xdot_t[3:] = np.array([0., 0., 0., 0.]) #todo, replace!!!
                x_t = np.array([np.append(np.append(x_ee, xee_orientation), xdot_t)])
            x_list[:, w] = x_t[0][0:self.params["dim_pos"]]
            ...
            if mode == "vel":
                ...
                action_safeMP_pulled = kuka_kinematics.get_qdot_from_pos_orient(q=q, xdot=action_safeMP)
            else:
            ...
            """
            x_t_NN_cpu = x_t_NN.T.cpu().detach().numpy().transpose()[0]
            x_goal_1_x = ob_robot['FullSensor']['goals'][nr_obst+3]['position']
            x_goal_2_z = ob_robot['FullSensor']['goals'][nr_obst+4]['position']
            rot_matrix = geometry_safeMP.get_rotation_matrix_from_quaternion(x_quat=x_t_NN_cpu[3:7])
            ...
            arguments_dict = dict(
                ...
                weight_goal_2=ob_robot['FullSensor']['goals'][nr_obst + 3]['weight'],
            ...
            ob, *_ = self.env.step(action_fabrics)
...
if __name__ == "__main__":
    ...
    res = example_class.run_kuka_example() #n_steps=1000, env=env, goal=goal, init_pos=init_pos, goal_pos=goal_pos,
                               #dt=dt, mode=mode, mode_NN=mode_NN, nr_obst=nr_obst)
