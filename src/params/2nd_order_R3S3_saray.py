...
class Params:
    ...
    """ Neural Network """
    latent_space_dim: int = 600  # dimensionality latent space, larger network ;)
    neurons_hidden_layers: int = 600  # number of neurons per layer, larger network ;)
    batch_size: int = 300  # sampling batch size, could be increased, the higher this number, the more stable the training.
    ...
    boundary_loss_weight: float = 1.0   # boundary loss weight
    imitation_window_size: int = 15  # imitation window size, #trained with 13, makes training slower (high number) but higher is more accurate.
    ...
    max_iterations: int = 4000  # maximum number of training iterations

    """ Preprocessing """
    spline_sample_type: str = 'from data'  # resample from spline type, options: from data (initial states gotten from demonstrations, bit biasing wrt the function of speed), evenly spaced (resample initial conditions evenly spaces in space (pos+vel).
