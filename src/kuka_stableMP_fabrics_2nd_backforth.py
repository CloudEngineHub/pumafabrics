...
class example_kuka_stableMP_R3S3():
    def __init__(self): #, bool_energy_regulator=False, bool_combined=True, robot_name="iiwa14"):
        ...
        self.solver_times = []
        with open("../../config/kuka_TamedPUMA_tomato.yaml", "r") as setup_stream:
    ...
    def run_kuka_example(self): #, n_steps=2000, goal_pos=[-0.24355761, -0.75252747, 0.5], mode="acc", mode_NN = "1st", dt=0.01, mode_env=None):
        ...
        # initial state:
        x_t_init = self.kuka_kinematics.get_initial_state_task(q_init=q_init, qdot_init=np.zeros((dof, 1)), offset_orientation=offset_orientation, params_name=self.params_name)
        ...
        for w in range(n_steps):
            ...
            x_t, xee_orientation, _ = self.kuka_kinematics.get_state_task(q, quat_prev, params_name=self.params_name, qdot=qdot)
...
if __name__ == "__main__":
    ...
    goal_pos = [0.47, 0.5, 0.8]
    example_class.overwrite_defaults(goal_pos=goal_pos)
