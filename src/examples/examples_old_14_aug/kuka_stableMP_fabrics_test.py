...
class example_kuka_stableMP_R3S3():
    def __init__(self, bool_energy_regulator=False, bool_combined=True, robot_name="iiwa7"):
    ...
    def relationship_dq_dx(self, offset_orientation, translation_cpu, kuka_kinematics, normalizations, fk):
        qq = ca.SX.sym("q", 7, 1)
        x_pose = kuka_kinematics.forward_kinematics_symbolic(end_link_name="iiwa_link_ee", fk=fk)
    ...
    def run_kuka_example(self, n_steps=2000, env=None, goal_pos=[-0.24355761, -0.75252747, 0.5], mode="acc", mode_NN = "1st", dt=0.01, mode_env=None):
        ...
        for w in range(n_steps):
            ...
            action_stableMP_pulled = kuka_kinematics.inverse_diff_kinematics_quat(xdot=xdot_pos_quat,
            ...
            qddot_stableMP = pdcontroller.control(desired_velocity=action_stableMP_pulled, current_velocity=qdot)
            ...
            action_list[:, w] = np.clip(action_stableMP_pulled, -vel_limits, vel_limits)
...
if __name__ == "__main__":
    ...
    params = {"render": render, "mode_env": mode_env, "dt":dt, "init_pos": init_pos, "goal_pos": goal_pos, "nr_obst": nr_obst, "robot_name": robot_name,
              "end_links": ["iiwa_link_ee"], "nr_obst_dyn": nr_obst, "positions_obstacles": [[0.5, -0.25, 0.5], [0.24355761, 0.45252747, 0.2]],
              "collision_links": []}
    (env, goal) = envir_trial.initialize_environment_kuka(params) #render, mode=mode_env, dt=dt, init_pos=init_pos,
                                                            #  goal_pos=goal_pos, nr_obst=nr_obst, robot_name=robot_name)
