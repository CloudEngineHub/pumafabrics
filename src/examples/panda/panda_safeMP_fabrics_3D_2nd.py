...
class example_panda_stableMP_fabrics_3D_2nd:
    def __init__(self):
        dt = 0.01
    ...
    def run_panda_example(self, n_steps=2000, env=None, goal=None, init_pos=np.array([0.0, 0.0, 0.]), goal_pos=[0.1, -0.6, 0.4], mode="acc", dt=0.01):
        ...
        for w in range(n_steps):
            ...
            ob, *_ = env.step(action_fabrics_safeMP) #action_safeMP_pulled) #safeMP_pulled) #action_fabrics_safeMP) #action_safeMP_pulled)

            # --- Update plot ---#
            trajectory_plotter.update(x_t_gpu.T.cpu().detach().numpy())
        plt.savefig(params.results_path+"images/panda_robot_plot_3D")
...
if __name__ == "__main__":
    ...
    example_class = example_panda_stableMP_fabrics_3D_2nd()
