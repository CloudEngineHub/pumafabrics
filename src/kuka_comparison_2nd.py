...
class comparison_kuka_class():
    def __init__(self):
        self.cases = ["PUMA$_free$", "PUMA$_obst$", "GF", "GM", "CM"]
    ...
    def KukaComparison(self, q_init_list:list, positions_obstacles_list:list, speed_obstacles_list:list, network_yaml: str, nr_obst:int):
        ...
        results_stableMP = self.run_i(class_SMP, case=self.cases[0], q_init_list=q_init_list, results_stableMP=None, positions_obstacles_list=positions_obstacles_list)

        # --- run safe MP (only) example ---#
        class_SMP_obst = example_kuka_stableMP_fabrics()
        class_SMP_obst.overwrite_defaults(bool_combined=False, nr_obst=nr_obst)
        results_stableMP_obst = self.run_i(class_SMP_obst, case=self.cases[1], q_init_list=q_init_list, results_stableMP=None, positions_obstacles_list=positions_obstacles_list)

        # --- run fabrics (only) example ---#
        class_fabrics = example_kuka_fabrics()
        class_fabrics.overwrite_defaults(nr_obst=nr_obst)
        results_fabrics = self.run_i(class_fabrics, case=self.cases[2], q_init_list=q_init_list, results_stableMP=results_stableMP, positions_obstacles_list=positions_obstacles_list)
        ...
        results_stableMP_fabrics = self.run_i(class_GM, case=self.cases[3], q_init_list=q_init_list, results_stableMP=results_stableMP, positions_obstacles_list=positions_obstacles_list)
        ...
        results_CM = self.run_i(class_CM, case=self.cases[4], q_init_list=q_init_list, results_stableMP=results_stableMP, positions_obstacles_list=positions_obstacles_list)

        self.results = {self.cases[0]: results_stableMP, self.cases[1]: results_stableMP_obst, self.cases[2]: results_fabrics, self.cases[3]: results_stableMP_fabrics, self.cases[4]: results_CM}
        return self.results
...
if __name__ == "__main__":
    ...
    kuka_class = comparison_kuka_class()
    if LOAD_RESULTS == False:
        # Regenerate the results:
        results = kuka_class.KukaComparison(q_init_list, positions_obstacles_list, speed_obstacles_list, network_yaml, nr_obst)
    else:
