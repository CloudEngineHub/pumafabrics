...
class example_kuka_stableMP_R3S3():
    ...
    def run_kuka_example(self, n_steps=2000, env=None, goal=None, init_pos=np.array([0.0, 0.0, 0.]), goal_pos=[-0.24355761, -0.75252747, 0.5], mode="acc", mode_NN = "1st", dt=0.01):
        ...
        orientation_goal = np.array([0.0726, -0.6326, 0.3511, -0.6865]) #np.array([1., 0., 0., 0.])
        ...
        x_init_gpu[0][3:7] = normalizations.system_quat_to_NN(quat=x_init_gpu[0][3:7], offset=offset_orientation)
        test =  normalizations.NN_quat_to_system(quat=x_init_gpu[0][3:7].cpu().numpy(), offset=-offset_orientation)
        if mode_NN == "2nd":
        ...
        for w in range(n_steps):
            ...
            if mode_NN == "2nd":
                ...
                xddot_t_NN = transition_info["phi"]
            action_safeMP = normalizations.reverse_transformation(action_gpu=action_t_gpu, dt=dt, mode_NN=mode_NN) #only use first three indices!!
            ...
            if mode_NN == "1st":
                ...
                ee_velocity_NN = np.append(action_safeMP[0:3], euler_vel/dt)
                action_safeMP_pulled_2 = kuka_kinematics.get_qdot_from_linear_angular_vel(q=q, xdot=action_safeMP_quat, x_pose=x_t[0])
                ...
                action_safeMP_pulled, action_quat_vel, euler_vel = cartesian_controller.control_law(position_d=x_t_action[:3], #state_goal,
                                                                                                    ...
                                                                                                    J=Jac_current)
                # print("difference:", np.linalg.norm(action_safeMP_pulled - action_safeMP_pulled_2))
            else:
                print("not implemented!!")
...
if __name__ == "__main__":
    ...
    goal_pos = [-0.07311596, -0.6, 0.4]# [-0.24355761, 0.75252747, 0.5] #[0.09206515, -0.55024947,  0.36955635] # [-0.24355761, -0.75252747, 0.5]
    ...
    res = example_class.run_kuka_example(n_steps=4000, env=env, goal=goal, init_pos=init_pos, goal_pos=goal_pos,
