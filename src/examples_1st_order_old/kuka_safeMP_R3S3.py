...
class example_kuka_stableMP_R3S3():
    ...
    def run_kuka_example(self, n_steps=2000, env=None, goal=None, init_pos=np.array([0.0, 0.0, 0.]), goal_pos=[-0.24355761, -0.75252747, 0.5], mode="acc", mode_NN = "1st", dt=0.01):
        ...
        if params_name[0] == '2':
            ...
            #np.append(x_function(q_init).full()[0:dim_pos], x_orientation)
            pose_t_init = kuka_kinematics.forward_kinematics(q_init)
            x_t_init = np.array([np.append(pose_t_init, xdot_init)])
        else:
            #x_orientation = np.array([1., 0., 0., 0])
...
if __name__ == "__main__":
    ...
    res = example_class.run_kuka_example(n_steps=3000, env=env, goal=goal, init_pos=init_pos, goal_pos=goal_pos,
