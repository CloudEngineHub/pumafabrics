...
#     """
#     robots = [
#         GenericUrdfReacher(urdf="examples/urdfs/iiwa7.urdf", mode="acc"),
#     ]
...
def set_planner(goal: GoalComposition, degrees_of_freedom: int = 7):
    ...
    planner.concretize_extensive(mode="acc", time_step=dt, extensive_concretize=False,
                                 bool_speed_control=True)
    return planner


def run_panda_example(n_steps=5000, render=True, env=None, goal=None):
...
if __name__ == "__main__":
    ...
    init_pos = np.array([0.0, 0.8, -1.5, 2.0, 0.0, 0.0, 0.0])
    nr_obst = 2
    robot_name = "iiwa7"
    ...
    res = run_panda_example(n_steps=5000, env=env, goal=goal)
